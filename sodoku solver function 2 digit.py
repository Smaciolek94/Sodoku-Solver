    ...
    for j in range(0,9): #then over all columns
        if math.isnan(puzzle[i,j]): #if the value is missing, create a possibility vector
            ...
            if len(possib) == 1:
                ...
                break
            if len(possib) == 2: #creating a possiblity 3-d array
                ...
                if np.count_nonzero(possible2[:,j,:]==n) == 2:
                    if n not in possible2[i,j,:]:
                        if n in possib:
                            possib.remove(n)
                        box(i,j,puzzle,possib,0,0)
            box2(i,j,puzzle,possib,possible2,0,0)
            box2(i,j,puzzle,possib,possible2,0,3)
            box2(i,j,puzzle,possib,possible2,0,6)
            box2(i,j,puzzle,possib,possible2,3,0)
            box2(i,j,puzzle,possib,possible2,3,3)
            box2(i,j,puzzle,possib,possible2,3,6)
            box2(i,j,puzzle,possib,possible2,6,0)
            box2(i,j,puzzle,possib,possible2,6,3)
            box2(i,j,puzzle,possib,possible2,6,6)
            if len(possib) == 1:
                puzzle[i,j] = int(possib[0])
possible2 = np.zeros((9,9,2))               
